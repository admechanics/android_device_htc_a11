...
public class A11RIL extends RIL implements CommandsInterface {
    ...
    processUnsolicited (Parcel p) {
        ...
        if (response == RIL_UNSOL_HTC_UICC_SUBSCRIPTION_STATUS_CHANGED) {
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
            ...
            case RIL_REQUEST_HTC_GET_DATA_CALL_PROFILE: ret =  responseGetDataCallProfile(p); break;
...
*/
        switch(request) {
            ...
            case RIL_REQUEST_HTC_GET_DATA_CALL_PROFILE: return "RIL_REQUEST_GET_DATA_CALL_PROFILE";
